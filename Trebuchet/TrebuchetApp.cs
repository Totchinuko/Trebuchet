...
namespace Trebuchet
{
    public class TrebuchetApp : INotifyPropertyChanged
    {
        ...
        public TrebuchetApp(Config config, UIConfig uiConfig)
        {
            Config = config;
            UiConfig = uiConfig;
            Steam = new SteamSession(Config);
            SteamWidget = new SteamWidget(Steam);
            Trebuchet = new TrebuchetLauncher(config);
        ...
        public void BaseChecks()
        {
            ...
            if (string.IsNullOrEmpty(Config.InstallPath))
                new MessageModal("Install Folder", "In order to use Trebuchet, please configure a folder to install your mods and profiles").ShowDialog();
